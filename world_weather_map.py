...
"""
use service OpenWeather (openweathermap.org)
...
df_coordinates = df.apply(lambda i: get_coordinates(i.city, i.country), axis=1)
...
# Convert pandas DataFrame into GeoDataFrame and create a geometry column with coordinates
gdf = gpd.GeoDataFrame(df, geometry=gpd.points_from_xy(df.longitude, df.latitude), crs=4326)

# ==============================================================================
# Now the map can be rendered using Geopandas and Matplotlib.
...
# add city name label
gdf.to_crs(epsg=3857).apply(
    lambda x: ax.annotate(
        text=f"{x.city}  ", fontsize=10, color="black", xy=x.geometry.centroid.coords[0], ha="right"
    ),
    axis=1,
)
...
gdf.to_crs(epsg=3857).apply(
    lambda x: ax.annotate(
        text=f" {round(x.temp)}°", fontsize=15, color="black", xy=x.geometry.centroid.coords[0], ha="left"
    ),
    axis=1,
)
